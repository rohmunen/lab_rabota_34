...
def generate_ch_password(m):
    ...
    while len(res_password) < m:
        if (ch := ascii_lowercase[random.randint(0, len(ascii_lowercase) - 1)]) not in res_password and ch.upper() not in res_password:
            ch.lower()
            if ch in ["i", "l"]:
                case = 0
            else:
...
def check(password):
    if len(re.findall('[A-Z]', password)) == 0 or len(re.findall('[a-z]', password)) == 0 or len(
            re.findall("[0-9]", password)) == 0:
        return False
    else:
        return True
